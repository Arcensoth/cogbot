...
class MCCQExtension:
    ...
    def get_version_label(self, version: str, query_manager: QueryManager) -> str:
        ...
        return self.state.version_labels.get(version, version).format(
    ...
    async def mcc(self, ctx: Context, command: str, query_manager: QueryManager):
        ...
        try:
            ...
            await self.bot.send_message(ctx.message.channel, "Invalid arguments for that command")
            return
            ...
            await self.bot.send_message(ctx.message.channel, "Failed to load data for that command")
            ...
            if self.should_warn_legacy(ctx, command):
                await self.bot.send_message(
                    ...
                    "command data. Try `mcc12` for manually-written legacy versions.".format(
            ...
            await self.bot.send_message(
                ctx.message.channel, "Invalid regex for that command (you may need to use escaping)"
            )
            return
        ...
        if not results:
            ...
            await self.bot.send_message(ctx.message.channel, "No results found for that command")
            return
    ...
    @commands.cooldown(
        MCCQExtensionState.DEFAULT_COOLDOWN_RATE,
        MCCQExtensionState.DEFAULT_COOLDOWN_PER,
